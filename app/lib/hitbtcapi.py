...
class Client(object):
    ...
    def new_order(self, client_order_id, symbol_code, side, quantity, price=None):
        ...
        response = self.session.put("%s/order/%s" % (self.url, client_order_id), data=data)

        if 200 <= response.status_code <= 300:
            return response.json()
        else:
            raise Exception('{} {}'.format(response.status_code, json.loads(response.content.decode('ascii'))))
    ...
    def get_transactions(self):
        ...
        return self.session.get("%s/account/transactions" % (self.url)).json()
